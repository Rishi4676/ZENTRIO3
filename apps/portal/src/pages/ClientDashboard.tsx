...
import { PdfInvoiceModal } from '../components/PdfInvoiceModal';
                  ...
                  {clientProjects.map(project => {
                    ...
                    return (
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-4 border-t border-slate-100 dark:border-slate-850/50">
                        ...
                        {/* Payment action & PDF Invoice */}
                        <div className="flex items-center space-x-2">
                          <button
                            onClick={() => {
                              setSelectedInvoice({
                                invoiceNumber: `INV-ZNT-${Math.floor(100000 + Math.random() * 900000)}`,
                                date: new Date().toISOString().split('T')[0],
                                dueDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
                                items: [
                                  { description: `${project.title} - Full Architecture`, qty: 1, rate: project.budget, amount: project.budget }
                                ],
                                taxRate: 18,
                                discount: 0,
                                status: project.status === 'completed' || project.status === 'delivered' ? 'PAID' : 'PENDING'
                              });
                              setIsInvoiceModalOpen(true);
                            }}
                            className="px-3.5 py-2 text-xs font-bold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition flex items-center space-x-1.5 cursor-pointer"
                          ...
                          <button
                            ...
                            className="px-4 py-2 text-xs font-bold text-white bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 rounded-xl shadow-md transition active:scale-95 cursor-pointer"
                          >
