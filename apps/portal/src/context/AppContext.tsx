...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [rawCurrentUser, setCurrentUser] = useState<User | null>(null);
  
  const currentUser = React.useMemo(() => {
    if (!rawCurrentUser) return null;
    if (rawCurrentUser.role === 'worker') {
      let workerAvatar = rawCurrentUser.avatar;
      const id = rawCurrentUser.id || rawCurrentUser.email;
      if (id === 'syedrashid_W1' || id === 'syed.r@zentrio.ai') workerAvatar = '/syed.jpg';
      else if (id === 'rishigesh_W2' || id === 'rishi@zentrio.ai') workerAvatar = '/rishi.png';
      else if (id === 'pushparaj_W3' || id === 'pushpa.r@zentrio.ai') workerAvatar = 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=400&q=80';
      return { ...rawCurrentUser, avatar: workerAvatar };
    }
  ...
  const [currentPage, setCurrentPage] = useState<string>(() => {
    ...
    // 2. Otherwise, check if user session is saved in sessionStorage or localStorage and restore it
  ...
  const loginWithGoogle = async (googleUser?: { uid?: string; name: string; email: string; picture?: string }) => {
    try {
      let gUid = googleUser?.uid;
