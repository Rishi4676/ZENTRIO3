        ...
        <main className="flex-grow">
          ...
          {activeTab === 'support' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="text-xl font-extrabold tracking-tight">Support Terminal</h2>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 font-medium">Communicate directly with engineers or file complaint sheets.</p>
                </div>
              </div>
              ...
              <div className="grid lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-4">
                  <div className="glass-card p-5 rounded-2xl border border-slate-200/50 dark:border-slate-850">
                    <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 mb-4">Raise Support Ticket</h3>
                    
                    {ticketSuccess && (
                      <div className="mb-4 p-3 rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] font-semibold border border-emerald-500/20">
                        {ticketSuccess}
                      </div>
                    )}
                    ...
                    <form onSubmit={handleTicketSubmit} className="space-y-3">
                      <div>
                        ...
                        <input
                          type="text"
                          required
                          placeholder="e.g. Webhook API error"
                          value={ticketSubject}
                      ...
                      <div>
                        ...
                        <textarea
                          required
                          rows={3}
                          placeholder="Provide stack trace or invoicing issue..."
                          value={ticketDesc}
                ...
                {/* Right Column: Ticket chat & list */}
                <div className="lg:col-span-2 space-y-4">
                  
                  {/* Ticket List */}
                  <div className="glass-card p-5 rounded-2xl border border-slate-200/50 dark:border-slate-850">
                    <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 mb-4">Active Support Logs ({clientTickets.length})</h3>
                    {clientTickets.length === 0 ? (
                      <div className="text-center py-8 text-xs text-slate-500 italic">No tickets raised. Your account pipeline is nominal.</div>
                    ) : (
                        ...
                        {clientTickets.map(t => (
                          ...
                          >
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">{t.id} | {t.category}</span>
                              <div className="flex items-center space-x-2">
                                <span className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded ${
                                  t.priority === 'high' ? 'bg-rose-500/10 text-rose-500' : 'bg-slate-100 text-slate-600'
                                }`}>
                                  {t.priority}
                                </span>
                      ...
                      {(() => {
                        ...
                        return (
                          <>
                            ...
                            {/* Chat messages */}
                            <div className="h-60 overflow-y-auto space-y-3 p-2 bg-slate-100/50 dark:bg-slate-950/20 rounded-xl">
                              {ticket.messages.map(msg => {
                                const isAdmin = msg.senderRole === 'admin' || msg.senderRole === 'worker';
                                return (
                            ...
                            <div className="flex space-x-2">
                              <input
                                ...
                                onKeyDown={(e) => e.key === 'Enter' && handleSendChatReply(ticket.id)}
                              ...
                              />
                              <button
                                onClick={() => handleSendChatReply(ticket.id)}
                                className="p-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow active:scale-95 transition"
                              >
