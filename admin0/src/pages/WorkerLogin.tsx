...
export const WorkerLogin: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const canonicalId = workerIdMap[idNorm] || idNorm;
    const worker = users.find(u => 
      (u.id.toLowerCase() === canonicalId.toLowerCase() || u.id.toLowerCase() === idNorm) && 
      u.role === 'worker'
    );
    if (worker) {
      const actualCanonicalId = worker.id;
      let avatarUrl = worker.avatar;
      if (actualCanonicalId === 'syedrashid_W1') avatarUrl = '/assets/syed.jpg';
      if (actualCanonicalId === 'rishigesh_W2') avatarUrl = '/assets/rishi.png';
      if (actualCanonicalId === 'pushparaj_W3') avatarUrl = 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=400&q=80';
      setMatchedWorker({
        ...worker,
        avatar: avatarUrl
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (!result.success) {
      setErrorMessage(result.error || 'Access denied. Incorrect worker token keys.');
    }
  };
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center px-4 py-12 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 transition-colors duration-300 overflow-hidden">
      ...
      <div className="w-full max-w-md glass-card rounded-3xl p-8 border border-slate-200/50 dark:border-slate-800/50 shadow-2xl relative z-10">
        ...
        {errorMessage && (
          <div className="mb-5 p-3.5 rounded-xl bg-rose-500/10 text-rose-600 dark:text-rose-400 text-xs font-semibold border border-rose-500/20">
            {errorMessage}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {/* Worker ID Field */}
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Worker Identifier ID</label>
