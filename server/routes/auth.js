...
const { dbHelper, auth } = require('../database/db');
...
router.post('/google-onboard', signupLimiter, csrfCheck, async (req, res) => {
  try {
    const { name, email, photoURL, uid, companyName, mobile, country, state } = req.body;
    if (!name || !email) {
      return res.status(400).json({ success: false, message: 'Name and Email are required' });
    }
    ...
    if (isNew) {
      ...
      // Trigger Telegram Notification Agent
      try {
        const { notifyNewClientSignup } = require('../services/telegramService');
        await notifyNewClientSignup({ name, email, companyName, provider: 'Google OAuth' });
      } catch (tgErr) {
        console.warn('Telegram Notification Agent warning:', tgErr.message);
      }
    } else {
      ...
      await dbHelper.users.save(user.id || user.email, user);
      await writeAuditLog(email, email, 'USER_LOGIN_GOOGLE', 'Logged in with Google OAuth', req);
    }
